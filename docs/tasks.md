...
Current state: The server owner supplied 12 existing role IDs, mapped in order to I through z. The opt-in integration now listens for DiscordSRV link/unlink events, requests sync on tier gain, sweeps linked accounts at startup and every five minutes, bounds dispatch to eight requests per second, retries failures, and persists unlink cleanup IDs across restart. Highest-only remains the provisional mode pending the owner's preference. Local build and test packaging pass; live DiscordSRV/JDA, Bedrock, role hierarchy, outage, and restart checks remain for the test server.
